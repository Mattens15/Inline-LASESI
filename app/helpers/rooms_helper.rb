module RoomsHelper
  ...
  def do_i_matter?
    @room = Room.find(params[:id])
    !current_user.nil? && (current_user.admin? || current_user.powers.exists?(room_id: @room.id))
  end
  ...
  def can_i_reserve?
    @room = Room.find(params[:id])
    current_user.nil? || (!do_i_have_powers? && !current_user.reservations.exists?(room_id: @room.id))
  end
  ...
  def do_i_have_superpower?
    @room = Room.find(params[:id])
    !current_user.nil? && ( current_user.admin? || @room.user.id = current_user.id )
  end
  ...
  def do_i_have_powers?
    @room = Room.find(params[:id])
    !current_user.nil? && current_user.powers.exists?(room_id: @room.id)
  end
